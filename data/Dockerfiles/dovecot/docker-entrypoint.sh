...
echo -n 'quota acl zlib mail_crypt mail_crypt_acl mail_log notify listescape replication lazy_expunge mail_pgp_storage' > /etc/dovecot/mail_plugins
echo -n 'quota imap_quota imap_acl acl zlib imap_zlib imap_sieve mail_crypt mail_crypt_acl notify listescape replication mail_log mail_pgp_storage' > /etc/dovecot/mail_plugins_imap
echo -n 'quota sieve acl zlib mail_crypt mail_crypt_acl notify listescape replication mail_pgp_storage' > /etc/dovecot/mail_plugins_lmtp
else
echo -e "\e[32mDetecting SKIP_FTS=n... enabling Flatcurve (FTS)\e[0m"
echo -n 'quota acl zlib mail_crypt mail_crypt_acl mail_log notify fts fts_flatcurve listescape replication lazy_expunge mail_pgp_storage' > /etc/dovecot/mail_plugins
echo -n 'quota imap_quota imap_acl acl zlib imap_zlib imap_sieve mail_crypt mail_crypt_acl notify mail_log fts fts_flatcurve listescape replication mail_pgp_storage' > /etc/dovecot/mail_plugins_imap
echo -n 'quota sieve acl zlib mail_crypt mail_crypt_acl fts fts_flatcurve notify listescape replication mail_pgp_storage' > /etc/dovecot/mail_plugins_lmtp
fi
...
driver = mysql
connect = "host=/var/run/mysqld/mysqld.sock dbname=${DBNAME} user=${DBUSER} password=${DBPASS}"
# pgp_storage_encrypt is reported as off whenever the domain no longer allows
# storage encryption, so disabling it for a domain takes effect on the next
...
# append or expunge. A mailbox locked for storing more than its quota
# (attributes.quota_lock_from) gets acl-quota-locked instead, which also lets
# the owner delete mail, move it to Trash and remove folders, so it can get
# back within the quota. NULL keeps the plugin defaults for every other mailbox.
user_query = SELECT CONCAT(JSON_UNQUOTE(JSON_VALUE(mailbox.attributes, '$.mailbox_format')), mailbox.mailbox_path_prefix, '%d/%n/${MAILDIR_SUB}:VOLATILEDIR=/var/volatile/%u:INDEX=/var/vmail_index/%u') AS mail, '%s' AS protocol, 5000 AS uid, 5000 AS gid, concat('*:bytes=', mailbox.quota) AS quota_rule, IF(IFNULL(domain.pgp_storage, '1') = '1', IF(IFNULL(domain.pgp_enforce, 'none') <> 'none' AND COALESCE(JSON_UNQUOTE(JSON_VALUE(mailbox.attributes, '\$.pgp_public_key')), '') <> '', '1', JSON_UNQUOTE(JSON_VALUE(mailbox.attributes, '\$.pgp_storage_encrypt'))), '0') AS pgp_storage_encrypt, JSON_UNQUOTE(JSON_VALUE(mailbox.attributes, '\$.pgp_failure_mode')) AS pgp_failure_mode, IF(mailbox.active = '3', IF(JSON_VALUE(mailbox.attributes, '\$.quota_lock_from') IS NULL, 'vfile:/etc/dovecot/acl-read-only', 'vfile:/etc/dovecot/acl-quota-locked'), NULL) AS acl, IF(mailbox.active = '3', 'yes', NULL) AS acl_globals_only FROM mailbox LEFT JOIN domain ON domain.domain = mailbox.domain WHERE mailbox.username = '%u' AND mailbox.active IN ('1', '2', '3')
