...
driver = mysql
connect = "host=/var/run/mysqld/mysqld.sock dbname=${DBNAME} user=${DBUSER} password=${DBPASS}"
user_query = SELECT CONCAT(JSON_UNQUOTE(JSON_VALUE(attributes, '$.mailbox_format')), mailbox_path_prefix, '%d/%n/${MAILDIR_SUB}:VOLATILEDIR=/var/volatile/%u:INDEX=/var/vmail_index/%u') AS mail, '%s' AS protocol, 5000 AS uid, 5000 AS gid, concat('*:bytes=', quota) AS quota_rule, JSON_UNQUOTE(JSON_VALUE(attributes, '\$.pgp_storage_encrypt')) AS pgp_storage_encrypt, JSON_UNQUOTE(JSON_VALUE(attributes, '\$.pgp_failure_mode')) AS pgp_failure_mode FROM mailbox WHERE username = '%u' AND (active = '1' OR active = '2')
