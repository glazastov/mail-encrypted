...
cat <<EOF > /opt/postfix/conf/sql/mysql_read_only_sender_access.cf
# Autogenerated by mailcow
user = ${DBUSER}
password = ${DBPASS}
hosts = unix:/var/run/mysqld/mysqld.sock
dbname = ${DBNAME}
# Read-only mailboxes may still use SOGo to view existing mail, but cannot
# submit mail as their primary address or one of their aliases. Only aliases
# delivering to the read-only mailbox alone are rejected, so an alias shared
# with other mailboxes stays usable by them.
query = SELECT 'REJECT Mailbox is read-only' FROM mailbox
  WHERE mailbox.active = '3'
    AND (mailbox.username = '%s'
      OR mailbox.username IN (SELECT goto FROM alias WHERE address = '%s')
      OR mailbox.username IN (SELECT CONCAT('%u', '@', target_domain) FROM alias_domain WHERE alias_domain = '%d'))
  LIMIT 1
EOF

cat <<EOF > /opt/postfix/conf/sql/mysql_quota_locked_recipient_access.cf
# Autogenerated by mailcow
user = ${DBUSER}
password = ${DBPASS}
hosts = unix:/var/run/mysqld/mysqld.sock
dbname = ${DBNAME}
# A mailbox locked for storing more than its quota is read-only, so it is not
# in the delivery maps. Its senders are told it is full, temporarily, rather
# than that it does not exist: their servers retry while the owner frees space.
query = SELECT '452 4.2.2 Mailbox is over quota' FROM mailbox
  WHERE mailbox.active = '3'
    AND JSON_VALUE(mailbox.attributes, '$.quota_lock_from') IS NOT NULL
    AND (mailbox.username = '%s'
      OR mailbox.username IN (SELECT goto FROM alias WHERE address = '%s')
      OR mailbox.username IN (SELECT CONCAT('%u', '@', target_domain) FROM alias_domain WHERE alias_domain = '%d'))
  LIMIT 1
